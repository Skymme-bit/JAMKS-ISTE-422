...
// AddressEndpoint: Main class for handling related functionality
// AddressEndpoint: This class defines the core functionality for address endpoint.
...
class AddressEndpoint extends baseEndpoint {
  ...
  private count_post(req: Request, res: Response, next: NextFunction) {
    addressService
      ...
      .catch((err) => {
        ...
        const statusCode = message.includes(ERROR_MISSING_SEARCH)
          ? 400
          : message.includes(ERROR_UNEXPECTED_RESPONSE)
          ? 500
          : message.includes(ERROR_FAILED_FETCH)
          ? 503
          : 500;
  ...
  private distance_post(req: Request, res: Response, next: NextFunction) {
    addressService
      ...
      .catch((err) => {
        const message = err.message || "Internal Server Error";
        const statusCode = message.includes(ERROR_MISSING_COORDINATES)
          ? 400
          : 500;
        res
          .status(statusCode)
  ...
  private city_post(req: Request, res: Response, next: NextFunction) {
    addressService
      ...
      .catch((err) => {
        const message = err.message || "Internal Server Error";
        const statusCode = message.includes(ERROR_ZIP_REQUIRED)
          ? 400
          : message.includes(ERROR_CITY_NOT_FOUND)
          ? 404
          : message.includes(ERROR_FAILED_FETCH)
          ? 503
          : 500;
