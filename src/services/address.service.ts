...
// Main class for handling related functionality
// This class defines the core functionality for address service.
...
class AddressService {
  ...
  /**
   * This method is responsible for executing the async operation.
  ...
  public async count(addressRequest?: any): Promise<any> {
    return new Promise<any>(async (resolve, reject) => {
      loggerService
        .info({ path: "/address/count", message: "Received count request" })
        .flush();

      if (!addressRequest.city && !addressRequest.zip) {
        loggerService
          .warning({ path: "/address/count", message: "Missing city or zip" })
          .flush();
        return reject(
          new Error(
      ...
      this.request(addressRequest)
        .then((response) => {
          ...
          // Checking condition to determine control flow
          ...
          loggerService
            .info({
              path: "/address/count",
              message: `Query returned ${response.length} result(s)`,
            })
            .flush();
          resolve({
        ...
        .catch((err) => {
          loggerService
            .error({
              path: "/address/count",
              message: `Count failed: ${(err as Error).message}`,
            })
            .flush();
  ...
  /** async: This method is responsible for executing the async operation.
   * Ensure proper input validation and error handling.
   */
    ...
    return new Promise<any>((resolve, reject) => {
      ...
      if (!lat1 || !lon1 || !lat2 || !lon2) {
        loggerService
          .warning({
            path: "/address/distance",
            message:
              "Missing coordinates in request. Required: lat1, lon1, lat2, lon2.",
          })
          .flush();

        return reject(
          new Error("Missing coordinates. Required: lat1, lon1, lat2, lon2.")
        );
      }
      ...
      try {
        ...
        loggerService
          .error({
            path: "/address/distance",
            message: `Unexpected distance calculation error: ${
  ...
  public async cityLookup(addressRequest?: any): Promise<any> {
    return new Promise<any>(async (resolve, reject) => {
      ...
      const isCanadianPostal =
        typeof zip === "string" &&
        /^[A-Za-z]\d[A-Za-z][ -]?\d[A-Za-z]\d$/.test(zip);

      if (
        !zip ||
        (!isUSZip && !isCanadianPostal) ||
        ...
        loggerService
          .warning({
            path: "/address/city",
            message:
              "Invalid or missing zip. Only valid U.S. or Canadian postal codes allowed, no additional fields",
          })
          .flush();
